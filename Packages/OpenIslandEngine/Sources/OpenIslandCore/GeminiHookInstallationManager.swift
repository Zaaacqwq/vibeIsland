...
/// Installs / removes the VibeIsland-managed hook block in Gemini CLI's
/// `~/.gemini/settings.json`, alongside a manifest recording the exact command
/// we wrote so uninstall can target only our entries. Mirrors the Cursor/Codex
...
public final class GeminiHookInstallationManager: @unchecked Sendable {
    ...
    public func status(hooksBinaryURL: URL? = nil) throws -> GeminiHookInstallationStatus {
        ...
        let managedCommand = manifest?.hookCommand
            ?? resolvedBinaryURL.map { GeminiHookInstaller.hookCommand(for: $0.path) }
        let managedHooksPresent = ((try? GeminiHookInstaller.uninstallSettingsJSON(
